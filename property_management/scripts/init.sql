...
-- PostGIS расширение для работы с геоданными (если понадобится в будущем)
...
-- Создание пользователей для каждого сервиса (опционально)
-- DO $$ 
-- BEGIN
--     IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'property_user') THEN
--         CREATE ROLE property_user LOGIN PASSWORD 'property_pass';
--     END IF;
--     IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'tenant_user') THEN
--         CREATE ROLE tenant_user LOGIN PASSWORD 'tenant_pass';
--     END IF;
--     IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'monitoring_user') THEN
--         CREATE ROLE monitoring_user LOGIN PASSWORD 'monitoring_pass';
--     END IF;
-- END $$;

-- Предоставление прав на схемы
-- GRANT USAGE, CREATE ON SCHEMA property_service TO property_user;
-- GRANT USAGE, CREATE ON SCHEMA tenant_service TO tenant_user;
-- GRANT USAGE, CREATE ON SCHEMA monitoring_service TO monitoring_user;
